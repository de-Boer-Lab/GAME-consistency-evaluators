...
def calculate_pearson_r(predictions_content):
    """
    ...
    if "error" in predictions_dict:
        print("No predictions were returned for this task -> skipping (pearson_r = NaN).")
        return None
    ...
    # row position, so the result is correct regardless of the order the Predictor
    # returns its predictions in.
    ...
    # Enforce all-or-nothing. A Predictor must score every sequence it was sent or
    # return an error -- it does not get to pick a subset. If any sequence is missing
    # its forward/RC partner, the prediction set is incomplete, so we refuse to compute
    # a correlation (return NaN) rather than silently scoring the paired remainder.
...
def calculate_and_save_metrics(saved_predictions_path, output_dir):
    """
    Compute the RC-consistency Pearson r and append one row to the summary file
    evaluation_summary_<EVALUATOR_NAME>.csv (tab-separated, append mode).
    """
    try:
        ...
        description = "Consistency point (Astro)"
