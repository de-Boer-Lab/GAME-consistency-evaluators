# Consistency Track Request Evaluator (K562)

A genomic sequence Evaluator that tests consistency of predictions between forward sequences and their reverse complements. This Evaluator requests track chromatin accessibility predictions for K562 cell type (homo_sapiens) and calculates Pearson correlation as a metric of Predictor reliability.
...
To get started with running the pre-built container, download from Hugging Face: https://huggingface.co/datasets/deBoerLab/Consistency_Track__GAME
...
apptainer run --containall \
  ...
  consistency_evaluator_track.sif <predictor_ip> <predictor_port> /predictions
```
...
apptainer run --containall \
  ...
  consistency_evaluator_track.sif 192.168.1.100 8000 /predictions
...
apptainer consistency_evaluator_track.sif evaluator.def
```
...
Consistency_Track_Request_K562	Consistency track (K562)	MyPredictor	20250125-143022.123456	pearson_r	0.945	[{...}]
```
...
Calculates the correlation between forward sequence track predictions and their reverse complement track predictions. Final Pearson correlation is reported as the mean of the calculated correlations for each sequence and reverse complement pair.
