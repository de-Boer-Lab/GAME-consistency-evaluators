...
def calculate_pearson_r_track(predictions_content: str):
    ...
    #this assumes that the order is matched for the forward and RC
    ...
    correlations = []
    for i in range(0, len(forward_predictions)):
        r, _ = pearsonr(forward_predictions['Predicted_Value'].iloc[i], RC_predictions['Predicted_Value'].iloc[i][::-1])
        correlations.append(r)

    print(f"Calculated Mean Pearson r: {np.mean(correlations)}") 
